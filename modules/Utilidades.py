...
class to_HTML():
    ...
    def list(self, elements=[], ordered=False):
        if elements:
            self.li_elements = []
            for i in range(0, len(elements)):
                self.li_elements.append("<li>{}</li>".format(elements[i]))
            self.li_elements = ''.join(self.li_elements)
            if ordered:
                self.HTML = "<ol style='font-size:16px;'>" + self.li_elements + "</ol>"
            else:
                self.HTML = "<ul style='font-size:16px;'>" + self.li_elements + "</ul>"
            return self.HTML
        else:
            print("No se incluyeron elementos para adjuntar a la lista.")
